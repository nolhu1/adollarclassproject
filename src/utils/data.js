export async function getEducators() {
    return Array(6).fill({
        nickname: 'Educator',
        avatar: 'https://randomuser.me/api/portraits/med/men/75.jpg',
        price: 1,
        experience: ['React', 'Angular', 'Vue', 'Java', 'C#', 'Python'],
        specialty: 'Frontend Developer',
        qualifications: 'ISTQB Foundation Level, Agile Tester, Scrum Master, SAFe Agilist, ITIL Foundation Certificate',
        acheivements: [
            'Built a custom compiler in C++ for a university project, supporting basic syntax and error handling',
            'Designed and implemented a blogging platform with user authentication, markdown support, and analytics dashboard',
            'Contributed to an open-source project with over 1,000 stars on GitHub'
        ],
        reviews: [
            {
                "reviewer": "Sarah El-Masry",
                "rating": 5,
                "date": "2025-08-12",
                "comment": "XYZ is a highly skilled developer with a sharp eye for detail. His communication is clear and professional, and he consistently delivers high-quality work ahead of deadlines."
            },
            {
                "reviewer": "Jonas Müller",
                "rating": 4,
                "date": "2025-07-30",
                "comment": "Worked with XYZ on a machine learning project. He was proactive, well-organized, and brought creative solutions to technical challenges. Would definitely collaborate again."
            },
        ]
    });
}

export async function getMetors(){
    return Array(6).fill({
        nickname: 'Metor',
        avatar: 'https://randomuser.me/api/portraits/med/men/75.jpg',
        experience: ['React', 'Angular', 'Vue', 'Java', 'C#', 'Python'],
        price: 1,
        specialty: 'Frontend Developer',
        qualifications: 'ISTQB Foundation Level, Agile Tester, Scrum Master, SAFe Agilist, ITIL Foundation Certificate',
        acheivements: [
            'Built a custom compiler in C++ for a university project, supporting basic syntax and error handling',
            'Designed and implemented a blogging platform with user authentication, markdown support, and analytics dashboard',
            'Contributed to an open-source project with over 1,000 stars on GitHub'
        ],
        reviews: [
            {
                "reviewer": "Sarah El-Masry",
                "rating": 5,
                "date": "2025-08-12",
                "comment": "XYZ is a highly skilled developer with a sharp eye for detail. His communication is clear and professional, and he consistently delivers high-quality work ahead of deadlines."
            },
            {
                "reviewer": "Jonas Müller",
                "rating": 4,
                "date": "2025-07-30",
                "comment": "Worked with XYZ on a machine learning project. He was proactive, well-organized, and brought creative solutions to technical challenges. Would definitely collaborate again."
            },
        ]
    });
}

export async function getConversationalists(){
    return Array(6).fill({
        nickname: 'Conversationalist',
        avatar: 'https://randomuser.me/api/portraits/med/men/75.jpg',
        price: 1,
        experience: ['React', 'Angular', 'Vue', 'Java', 'C#', 'Python'],
        specialty: 'Frontend Developer',
        qualifications: 'ISTQB Foundation Level, Agile Tester, Scrum Master, SAFe Agilist, ITIL Foundation Certificate',
        acheivements: [
            'Built a custom compiler in C++ for a university project, supporting basic syntax and error handling',
            'Designed and implemented a blogging platform with user authentication, markdown support, and analytics dashboard',
            'Contributed to an open-source project with over 1,000 stars on GitHub'
        ],
        reviews: [
            {
                "reviewer": "Sarah El-Masry",
                "rating": 5,
                "date": "2025-08-12",
                "comment": "XYZ is a highly skilled developer with a sharp eye for detail. His communication is clear and professional, and he consistently delivers high-quality work ahead of deadlines."
            },
            {
                "reviewer": "Jonas Müller",
                "rating": 4,
                "date": "2025-07-30",
                "comment": "Worked with XYZ on a machine learning project. He was proactive, well-organized, and brought creative solutions to technical challenges. Would definitely collaborate again."
            },
        ]
    });
}

export async function getTrainersCoach(){
    return Array(6).fill({
        nickname: 'Trainer Coach',
        avatar: 'https://randomuser.me/api/portraits/med/men/75.jpg',
        price: 1,
        experience: ['React', 'Angular', 'Vue', 'Java', 'C#', 'Python'],
        specialty: 'Frontend Developer',
        qualifications: 'ISTQB Foundation Level, Agile Tester, Scrum Master, SAFe Agilist, ITIL Foundation Certificate',
        acheivements: [
            'Built a custom compiler in C++ for a university project, supporting basic syntax and error handling',
            'Designed and implemented a blogging platform with user authentication, markdown support, and analytics dashboard',
            'Contributed to an open-source project with over 1,000 stars on GitHub'
        ],
        reviews: [
            {
                "reviewer": "Sarah El-Masry",
                "rating": 5,
                "date": "2025-08-12",
                "comment": "XYZ is a highly skilled developer with a sharp eye for detail. His communication is clear and professional, and he consistently delivers high-quality work ahead of deadlines."
            },
            {
                "reviewer": "Jonas Müller",
                "rating": 4,
                "date": "2025-07-30",
                "comment": "Worked with XYZ on a machine learning project. He was proactive, well-organized, and brought creative solutions to technical challenges. Would definitely collaborate again."
            },
        ]
    });
}

export async function getCounselors(){
    return Array(6).fill({
        nickname: 'Counselor',
        avatar: 'https://randomuser.me/api/portraits/med/men/75.jpg',
        price: 1,
        experience: ['React', 'Angular', 'Vue', 'Java', 'C#', 'Python'],
        specialty: 'Frontend Developer',
        qualifications: 'ISTQB Foundation Level, Agile Tester, Scrum Master, SAFe Agilist, ITIL Foundation Certificate',
        acheivements: [
            'Built a custom compiler in C++ for a university project, supporting basic syntax and error handling',
            'Designed and implemented a blogging platform with user authentication, markdown support, and analytics dashboard',
            'Contributed to an open-source project with over 1,000 stars on GitHub'
        ],
        reviews: [
            {
                "reviewer": "Sarah El-Masry",
                "rating": 5,
                "date": "2025-08-12",
                "comment": "XYZ is a highly skilled developer with a sharp eye for detail. His communication is clear and professional, and he consistently delivers high-quality work ahead of deadlines."
            },
            {
                "reviewer": "Jonas Müller",
                "rating": 4,
                "date": "2025-07-30",
                "comment": "Worked with XYZ on a machine learning project. He was proactive, well-organized, and brought creative solutions to technical challenges. Would definitely collaborate again."
            },
        ]
    });
}


export async function getIBAREs(){
    return Array(6).fill({
        nickname: 'iBARE',
        avatar: 'https://randomuser.me/api/portraits/med/men/75.jpg',
        price: 1,
        experience: ['React', 'Angular', 'Vue', 'Java', 'C#', 'Python'],
        specialty: 'Frontend Developer',
        qualifications: 'ISTQB Foundation Level, Agile Tester, Scrum Master, SAFe Agilist, ITIL Foundation Certificate',
        acheivements: [
            'Built a custom compiler in C++ for a university project, supporting basic syntax and error handling',
            'Designed and implemented a blogging platform with user authentication, markdown support, and analytics dashboard',
            'Contributed to an open-source project with over 1,000 stars on GitHub'
        ],
        reviews: [
            {
                "reviewer": "Sarah El-Masry",
                "rating": 5,
                "date": "2025-08-12",
                "comment": "XYZ is a highly skilled developer with a sharp eye for detail. His communication is clear and professional, and he consistently delivers high-quality work ahead of deadlines."
            },
            {
                "reviewer": "Jonas Müller",
                "rating": 4,
                "date": "2025-07-30",
                "comment": "Worked with XYZ on a machine learning project. He was proactive, well-organized, and brought creative solutions to technical challenges. Would definitely collaborate again."
            },
        ]
    });
}